    ...
    public class SnapAnchorsWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            switch (mode)
            {
                ...
                case AnchorMode.Point:
                    ...
                    if(GUILayout.Toggle(usePivot, "Pivot", "Radio"))
                    {
                        usePivot = true;
                    }

                    if (GUILayout.Toggle(!usePivot, "Custom", "Radio"))
                    {
                        usePivot = false;
                    }
                    ...
                    EditorGUI.BeginDisabledGroup(usePivot);
                    point = EditorGUILayout.Vector2Field("Relative Position", point);
                    EditorGUI.EndDisabledGroup();
        ...
        Vector2 GetPivotOffset(RectTransform obj)
        {
            ...
            if (parentPosition && mode == AnchorMode.Point)
            {
                ...
                Vector2 p = (usePivot) ? parentTransform.pivot : point;
        ...
        void DrawPointButtons()
        {
            ...
            const float size = 18;
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(" ┌", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0f, 1f);
            if (GUILayout.Button(" ┬", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0.5f, 1f);
            if (GUILayout.Button(" ┐", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(1f, 1f);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(" ├", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0f, 0.5f);
            if (GUILayout.Button(" ┼", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0.5f, 0.5f);
            if (GUILayout.Button(" ┤", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(1f, 0.5f);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(" └", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0f, 0f);
            if (GUILayout.Button(" ┴", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(0.5f, 0f);
            if (GUILayout.Button(" ┘", style, GUILayout.Width(size), GUILayout.Height(size))) SetPoint(1f, 0f);
            EditorGUILayout.EndHorizontal();
